...
<head>
  ...
  <style>
    ...
    img.thumb { max-width: 40px; max-height: 40px; display: block; margin: auto; border: 1px solid #aaa; border-radius: 3px; }
    .preview-row { display: none; background-color: #f9f9f9; }
    .preview-content { padding: 10px; border-left: 4px solid #00aaff; background: #fcfcfc; }
  </style>
</head>
<body>
...
<label style="margin-left: 20px;"><input type="checkbox" id="showAllPreview" /> Hiện xem trước toàn bộ</label>
      ...
      <tr class="preview-row">
        <td colspan="9">
          <div class="preview-content">
            <div><strong>Câu hỏi:</strong> <span class="mj"><?= $row["question"] ?></span></div>
            <div><strong>Đáp án A:</strong> <span class="mj"><?= $row["answer1"] ?></span></div>
            <div><strong>Đáp án B:</strong> <span class="mj"><?= $row["answer2"] ?></span></div>
            <div><strong>Đáp án C:</strong> <span class="mj"><?= $row["answer3"] ?></span></div>
            <div><strong>Đáp án D:</strong> <span class="mj"><?= $row["answer4"] ?></span></div>
          </div>
        </td>
      </tr>
    <?php endforeach; ?>
...
function togglePreview(row) {
  ...
  if (next && next.classList.contains('preview-row')) {
    ...
    if (!isVisible) MathJax.typesetPromise();
  }
}
...
$(document).ready(function () {
  const table = $('#questionTable').DataTable({
    ...
    language: {
      ...
      paginate: {
        first: "Đầu", last: "Cuối", next: "Sau", previous: "Trước"
      },
      buttons: {
        copy: "Sao chép", csv: "Xuất CSV", excel: "Xuất Excel", pdf: "Xuất PDF", print: "In"
      }
    }
  });
  ...
  $('#showAllPreview').on('change', function () {
    const show = this.checked;
    $('.preview-row').each(function () {
      this.style.display = show ? 'table-row' : 'none';
    });
    if (show) MathJax.typesetPromise();
  });

  table.on('draw', () => MathJax.typesetPromise());
  setTimeout(() => {
    const firstRow = document.querySelector("tbody tr:not(.preview-row)");
